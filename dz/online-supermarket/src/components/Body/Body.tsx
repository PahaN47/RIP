...
export const Body = ({ children }: BodyProps) => {
  ...
  useEffect(() => {
    if (cartFound) {
      if (!cartSelling && user && !user.is_staff) dispatch(getCart(user.id));
    } else if (!cartSelling && user && !user.is_staff)
  ...
  return (
    <ContentWrapStyled>
      ...
      <OrdersLinkWrapStyled>
        {user ? (
          <>
            <OrdersLinkStyled to={ORDERS_LINK}>
              {user.is_staff ? "Заказы" : "Мои заказы"}
            </OrdersLinkStyled>
            {!user.is_staff && (
